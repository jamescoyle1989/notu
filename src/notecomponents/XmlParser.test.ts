...
test('parseNoteXml correctly splits up text into elements', () => {
    ...
    expect(testElem.length).toBe(37);
...
test('parseNoteXml can correctly process a nested xml element', () => {
    ...
    expect(test2.length).toBe(25);
});


test('parseNoteXml handles unescaped angle brackets within text', () => {
    const result = parseNoteXml(`I think 2 < 1, but I also would like to <|Test1/|> whether 2 > 3`);

    expect(result.length).toBe(3);

    expect(result[0]).toBe('I think 2 < 1, but I also would like to ');

    const test1 = result[1] as NoteXmlElement;
    expect(test1.tag).toBe('Test1');
    expect(test1.children).toHaveLength(0);
    expect(test1.length).toBe(10);

    expect(result[2]).toBe(' whether 2 > 3');
});